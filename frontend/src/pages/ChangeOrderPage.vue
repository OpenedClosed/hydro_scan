  ...
  <q-page padding id="orderPage">
    <q-form @submit="changeOrder()">
      <div class="q-pa-xl backgroundStyle">
        ...
        <div>
          <q-btn type="submit">Редактировать заказ</q-btn>
        </div>
      </div>
    </q-form>
  </q-page>
...
function initiateData() {
  ...
  apiAxios
    ...
    .then((response) => {
      characteristics_group.value = response.data.characteristic_group
      bottom_group.value = response.data.bottom_group
      name_text.value = response.data.name
      service_group.value = response.data.service_group
      latitude = response.data.map_latitude
      longitude = response.data.map_longitude
      surface_of_research_text.value = response.data.research_surface
      water_turbidity_text.value = response.data.water_turbidity
      flow_text.value = response.data.flow
      deadline_date.value = response.data.deadline
      additional_description_text.value = response.data.additional_description

    })

  // console.log(inputData.value)

  if (!(deadline_date.value.length > 10)){
    deadline_date.value = deadline_date.value + "T00:00:00+03:00"
  }
  const data = {
...
function sendOrder(data) {
    ...
    const value = route.query.data
    apiAxios.put(`/api/orders/${value}/`, data)
    .then(
      router.push("/dashboard").then(() => {
        window.location.reload();
      })
